        ...
        if(zz1>0){
            try{
                data.map((e,i)=>{
                    ...
                    let docId =  e?.userId || '-';
                    let mobile = `${e?.mobile || '-'}`;
                    let status = `<span class="badge ${(e?.status.toLowerCase()=='active'.toLowerCase())? 'bg-success' : 'bg-danger'}">${capitalizeFirstLetter(e?.status || 'Pending')}</span>`
                ...
                row = `<tr><td colspan="5" class="text-center">Got error, please try again later!</td></tr>`;
                console.log("data not found : - ",error);
            }
        } else {
            row = `<tr><td colspan="5" class="text-center">Data Not Found.</td></tr>`;
        }        
        tbody.innerHTML = row;
        checkbox_function();
    } catch(error){
