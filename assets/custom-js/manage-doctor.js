...
function renderTable(data) {
    ...
    try {
        ...
        data.forEach((e) => {
            if((e?.userId || '-').toLowerCase().includes("dr")){
                let _id = e?._id || "-";
                ...
                row += `
                    <tr data-id='${_id}'>
                        ...
                        <td>${status}</td>
                        <td class="text-end">
                            <a href="view-doctor.html?id=${_id}" class="btn btn-sm btn-success"><i class="ri-eye-line"></i></a>
                            <a href="edit-doctor.html?id=${_id}" class="btn btn-sm btn-primary employee_restriction_d_none"><i class="ri-pencil-line"></i></a>
...
async function fetchDoctor() {
    try {
        const searchQuery = document.getElementById("doctor-search").value.trim();
...
async function all_data_load_list() {
    ...
    try{
        ...
        const data = r1?.doctors;
        let zz1 = r1?.totalDoctors;

        renderTable(data || []);
        setTotalDataCount(zz1 || 0);
        ...
        if (zz1 > 0) {
            ...
            row = `<tr><td colspan="6" class="text-center">Data Not Found.</td></tr>`;
        }
...
document.getElementById("doctor-search").addEventListener("input", fetchDoctor);
...
document.getElementById("edit_status").addEventListener("submit", async function (event) {
    ...
    try {
        ...
        const response = await fetch(EMPLOYEE_API_UPDATE, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: `${TOKEN}`,
        ...
        if (response.ok) {
            all_data_load_list(); // Reload full data to reflect changes
        }
    } catch (error) {
