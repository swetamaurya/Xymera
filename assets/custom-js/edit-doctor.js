...
async function editData() {
    ...
    try{
        ...
        speciality.value = r2?.speciality || '' ;
        ...
        try{
            let [time1From, time1To] = (r2?.doctor_time1 || '' ).split(" to ");
            let [time2From, time2To] = (r2?.doctor_time2 || '').split(" to ");
...
document.getElementById("edit-doctor").addEventListener("submit", async function(event){
    ...
    try{
        ...
        setTimeout(function(){
            window.location.href = 'manage-doctors.html'; 
            // history.back();
        },1000);
    } catch(error){
        console.log(error);
...
function rtnFormData(){
    ...
    f1.append("doctor_time1",d_t_1_final);
    f1.append("doctor_time2",d_t_2_final);
    f1.append("visitFrequency",noOfFeq);
    f1.append("category",categoryyy);
    f1.append("speciality",speciality);
    f1.append("onboarding",doob);
    f1.append("status", status_role);
    f1.append("roles","Doctor");
